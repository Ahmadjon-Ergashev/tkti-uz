...
class BRMItemsDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        connected_news = news.News.objects.filter(
            status=widgets.Status.published, brm=object.id).order_by("-added_at")
        paginator = Paginator(connected_news, 9)
        page_number = self.request.GET.get("page", 1)
        paginated_objects = paginator.get_page(page_number)
        context["connected_news"] = paginated_objects

        pagination_end_number = paginated_objects.number + 5 if paginated_objects.number + 5 < paginator.num_pages else paginator.num_pages + 1
        if paginated_objects.number < 6:
            pagination_end_number += 6-paginated_objects.number
        context["page_range"] = range(1 if pagination_end_number - 10 < 1 else pagination_end_number - 10, pagination_end_number)
